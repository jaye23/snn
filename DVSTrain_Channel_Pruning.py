...
def main():
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        # 判断是否重新激活
        if test_acc >= reactivation_acc_threshold and not reactivation_done:
            print(
                f"\n训练准确率 ({test_acc:.4f}) 首次达到阈值 ({reactivation_acc_threshold:.2f})，神经元再激活...")

            # 调用模型自身的再激活方法
            net.reactivate_fading_neurons(fsr_threshold=fsr_threshold)

            # 关键步骤：执行后立即更新标志位，防止重复调用
            reactivation_done = True
    ...
    if reactivation_epoch != -1:
        # 找到再激活那个epoch对应的训练准确率
        # epoch_list 中的索引和 train_acc_list 中的索引是一一对应的
        # (假设 epoch_list 是从0开始连续的，即 epoch_list[i] == i)
        acc_at_reactivation = test_acc_list[reactivation_epoch - start_epoch]  # 修正索引，如果start_epoch不为0

        plt.scatter(reactivation_epoch, acc_at_reactivation,
                    color='red',  # 标记点的颜色
                    s=100,  # 标记点的大小
                    zorder=5,  # 确保标记点绘制在折线的上方
                    label=f'Reactivation at Epoch {reactivation_epoch}')  # 为标记点添加图例
        #
        # # 可选：在标记点旁边添加文字注释
    ...
    plt.grid(True)
    plt.tight_layout()
    plt.savefig(f'test_acc_snr_{args.snr}_pruning.png')  # 可选，保存图像
    plt.show()
