...
class DVSGestureNet(nn.Module):
    def __init__(self, channels, spiking_neuron: callable = None, PDP=None, snr=None, device=None, *args, **kwargs):
        ...
        self.spike_counts = torch.zeros(channels * 4 * 4, device=device)
        self.time_steps = 0
        ...
        self.flatten = layer.Flatten()  # 把 CNN 特征展平
        self.dropout = layer.Dropout(0.5)  # 训练时随机丢弃 50% 神经元，防止过拟合

        self.fc = layer.Linear(channels * 4 * 4, 512,bias=False)#([16, 16, 512])
    ...
    def forward(self, x: torch.Tensor):
        ...
        total_tbn = x.numel()
        total_spikes_in_batch_1 = total_spikes_in_batch / total_tbn
        ...
        total_spikes_in_batch = x.sum()
        total_tbn = x.numel()
        total_spikes_in_batch_2 = total_spikes_in_batch / total_tbn

        total_spikes_in_batch_a = total_spikes_in_batch_1 + total_spikes_in_batch_2
        ...
        return x,total_spikes_in_batch_a,neuron_output
