...
import DVSNet_Channel_Pruning_Period_fisher2
...
def main():
    ...
    net = DVSNet_Channel_Pruning_Period_fisher2.DVSGestureNet(channels=args.channels, spiking_neuron=neuron.LIFNode,
    ...
    start_epoch = 0
    try:
        start_epoch, _ , train_acc_list, test_acc_list, epoch_list,spearman_list, pearson_list= load_checkpoint(net, optimizer, lr_scheduler, f'checkpoints_{args.r}/checkpoint_epoch_150.pth')
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        if not pruning_started and test_acc >= args.prune_acc_threshold and epoch >= 150:
            ...
            # 启动第一次剪枝
            num_retained, num_pruned,fsr_per_neuron,pruned_indices = net.prune_fading_neurons(fsr_threshold=current_fsr_threshold)
            prune_check = 1
            ...
            log_fisher_fsr_statistics(fisher_info, fsr_per_neuron, pruned_indices,
                                      epoch, pruning_count,
                                      save_dir=f'fisher_fsr_vs_{args.s}/',
            ...
            if num_retained > 60:
                ...
                if train_acc >= 0.98:
                    # add
                    fisher_info = compute_fisher_information(net, train_data_loader, args.device)
                    num_retained, num_pruned,fsr_per_neuron,pruned_indices = net.prune_fading_neurons_num(prune_num=10 + num_pruned)
                    prune_check = 1
                    test_loss, test_acc, test_speed = evaluate(net, test_data_loader, args.device,train_time)  # 你训练里的测试函数
                    pruning_count, last_prune_acc = record_pruning_info(epoch, pruning_count, num_pruned, num_retained,
                                                                        last_prune_acc, test_loss, test_acc, test_speed,
                ...
                if train_acc >= 0.98:
                    # add
                    fisher_info = compute_fisher_information(net, train_data_loader, args.device)
                    num_retained, num_pruned,fsr_per_neuron,pruned_indices = net.prune_fading_neurons_num(prune_num=5 + num_pruned)
                    prune_check = 1
                    test_loss, test_acc, test_speed = evaluate(net, test_data_loader, args.device,train_time)  # 你训练里的测试函数
                    pruning_count, last_prune_acc = record_pruning_info(epoch, pruning_count, num_pruned, num_retained,
                                                                        last_prune_acc, test_loss, test_acc, test_speed,
            ...
            elif 30 >= num_retained > 10:
                # if (epoch - first_prune_epoch) % 30 == 0 and train_acc >= 0.98:
                if train_acc >= 0.98:
                    # add
                    fisher_info = compute_fisher_information(net, train_data_loader, args.device)
                    num_retained, num_pruned,fsr_per_neuron ,pruned_indices= net.prune_fading_neurons_num(prune_num=3 + num_pruned)
                    prune_check = 1
                    test_loss, test_acc, test_speed = evaluate(net, test_data_loader, args.device,train_time)  # 你训练里的测试函数
                    pruning_count, last_prune_acc = record_pruning_info(epoch, pruning_count, num_pruned, num_retained,
                                                                        last_prune_acc, test_loss, test_acc, test_speed,
        ...
        if epoch == 150:
            save_checkpoint(net, optimizer, epoch, loss, lr_scheduler, dir_path=f'checkpoints_{args.s}/',
                            ...
                            spearman_list=spearman_list, pearson_list=pearson_list)
    ...
    plt.tight_layout()
    plt.savefig(
        f'acc_snr_{args.snr}_start_fsr_{args.fsr_threshold}_lambda_{args.regular_lambda}_channel_prune_period_slow_limit_{args.s}.png')
    plt.show()
    ...
    plt.tight_layout()
    plt.savefig(
        f'test_acc_vs_pruned_neurons_snr_{args.snr}_start_fsr_{args.fsr_threshold}_lambda_{args.regular_lambda}_period_slow_limit_{args.s}.png')
    plt.show()
    ...
    plt.tight_layout()
    plt.savefig(
        f'test_acc_vs_retained_neurons_snr_{args.snr}_start_fsr_{args.fsr_threshold}_lambda_{args.regular_lambda}_period_slow_limit_{args.s}.png')
    plt.show()
