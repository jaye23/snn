...
def main():
    ...
    net = DVSNet_Channel_ann.DVSGestureNet(channels=args.channels, spiking_neuron=neuron.LIFNode, surrogate_function=surrogate.ATan(), detach_reset=True,PDP=PDP,snr=args.snr,device=args.device)
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        for frame, label in train_data_loader:
            ...
            #ADD
            T, B = frame.shape[:2]
            frame = frame.reshape(T * B, *frame.shape[2:])  # 替代 view()
            ...
            if scaler is not None:
                with amp.autocast():
                    ...
                    #ADD
                    out = net(frame)  # [T*B, num_classes]
                    out = out.view(T, B, -1)  # [T, B, num_classes]
                    out_fr = out.mean(0)  # [B, num_classes]
