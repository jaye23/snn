...
import DVSNet_Channel_fisher_flat
from functions_fisher_flat import *
...
def main():
    ...
    net = DVSNet_Channel_fisher_flat.DVSGestureNet(channels=args.channels, spiking_neuron=neuron.LIFNode,
    ...
    start_epoch = 0
    try:
        start_epoch, _, train_acc_list, test_acc_list, epoch_list = load_checkpoint(net, optimizer, lr_scheduler, 'checkpoints_fisher/checkpoint_epoch_359.pth')
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        if (epoch+1) % 20 == 0 and epoch >= 99:
            save_checkpoint(net, optimizer, epoch, loss, lr_scheduler, dir_path=f'checkpoints_fisher_{args.s}/',
        ...
        if epoch % 20 == 0 and epoch >= 100:
            ...
            fsr_per_neuron=net.fsr()
            # print('fsr:', fsr_per_neuron.shape)
            log_fisher_fsr_statistics(fisher_info, fsr_per_neuron,
                                      epoch, save_dir=f'fisher_vs_fsr_SVD_flat_{args.s}/',
    ...
    plt.tight_layout()
    plt.savefig(
        f'acc_snr_{args.snr}_lambda_{args.regular_lambda}_channel.png')
    plt.show()
